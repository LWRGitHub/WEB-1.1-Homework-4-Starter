...
def create():
    ...
    if request.method == 'POST':
        #New plant's name, variety, photo, & date planted, and 
...
def detail(plant_id):
    ...
    # `find` database operation to find all harvests for the
    # plant's id.
    # This query is on the `harvests` collection, not the `plants`
    # collection.
...
def harvest(plant_id):
    ...
    # Creating a new harvest object by passing in the form data from the
    # detail page form.
    new_harvest = {
...
def edit(plant_id):
    ...
    if request.method == 'POST':
        # `update_one` database call to update the plant with the
        # given id. Updated fields are in the `$set` object.
        plant = {
            'name': request.form.get('plant_name'),
            'variety': request.form.get('variety'),
            'photo_url': request.form.get('photo'),
        ...
        # `find_one` database call to get the plant object with the
        # passed-in _id.
        plant_to_show = mongo.db.plants.find_one({'_id': ObjectId(plant_id)})
        # print(plant_to_show)
        context = {
...
def delete(plant_id):
    # `delete_one` database call to delete the plant with the given
    # id.
    mongo.db.plants.delete_one({'_id': ObjectId(plant_id)})

    # `delete_many` database call to delete all harvests with
    # the given plant id.
